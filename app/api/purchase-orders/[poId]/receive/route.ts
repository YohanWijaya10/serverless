    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (parsed.items) {
        for (const it of parsed.items)
          receiveMap.set(it.itemId, it.qtyToReceive);
      }

      for (const item of items) {
        const outstanding =
          Number(item.qtyOrdered) - Number(item.qtyReceived ?? 0);
        if (outstanding <= 0) continue;
        const qtyToReceive = parsed.items
          ? Math.min(outstanding, receiveMap.get(item.id) ?? 0)
          : outstanding;
      ...
      const refreshedItems = await (tx as any).purchaseOrderItem.findMany({
        where: { poId },
      });
      const allReceived = refreshedItems.every(
        (it: any) => Number(it.qtyReceived ?? 0) >= Number(it.qtyOrdered)
      );
